...
- **Actor**: This component is responsible for selecting actions. It learns the policy $\pi_{\mathbf{\theta}}(a|s)$ to maximize the expected return (which includes the reward and the weighted average expected value of future steps). This is done by adjusting $\mathbf{\theta}$ in the direction that increases the likelihood of actions that result in higher returns.

- **Critic**: This component evaluates the actions taken by the actor by learning a value function. The value function can be either the state-value function, $\mathcal{V}\_{\mathbf{w}}(s)$, which estimates the expected return (total accumulated reward) starting from state $s$, or the action-value function, $Q_{\mathbf{w}}(s, a)$, which estimates the expected return starting from state $s$ and taking action $a$. The critic aims to improve the accuracy of these estimated values. Minimizing the Temporal Difference (TD) error is a promising approach for achieving this. The TD error is defined as follows:

$$\delta_t = r_t + \gamma \mathcal{V}\_{\mathbf{w}}(s_{t+1}) - \mathcal{V}\_{\mathbf{w}}(s_{t})$$
  
where $\mathcal{V}\_{\mathbf{w}}(s_{t})$ is the value of the current state, $\mathcal{V}\_{\mathbf{w}}(s_{t+1})$ is the value of the next state, $r_t$ is the reward received, and $\mathbf{w}$ is the weight vector of the neural network learning the value function.
